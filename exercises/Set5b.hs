...
treeSize :: Tree a -> Int
treeSize t = treeSize' t 0
  where
    treeSize' Empty size = size
    treeSize' (Node a Empty r) size = treeSize' r (size + 1)
    treeSize' (Node a l Empty) size = treeSize' l (size + 1)
    treeSize' (Node a l r) size = 1 + size + (treeSize' l 0) + (treeSize' r 0)
...
cull _ Empty = Empty
cull val (Node a t1 t2) = if (val == a) then Empty else (Node a (cull val t1) (cull val t2))
...
walk (x:xs) (Node a t1 t2) = case x of
                                StepL -> walk xs t1
                                StepR -> walk xs t2
...
set (StepL:steps) val (Node a t1 t2) = Node a (set steps val t1) t2
set (StepR:steps) val (Node a t1 t2) = Node a t1 (set steps val t2)
...
search val (Node a t1 t2) = if val == a then Just []
                            else case search val t1 of
                                 Just steps -> Just $ StepL : steps
                                 Nothing -> case search val t2 of
