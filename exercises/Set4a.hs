...
allEqual [] = True
allEqual (x:xs) = foldl (\acc elem -> acc && (elem==x)) True xs
...
average xs = foldl (\acc elem -> elem + acc) 0 xs / fromIntegral (length xs)
...
freqs xs = let f Nothing = Just 1 
               f (Just x) = Just (x+1)
            in foldr (\elem acc -> Map.alter f elem acc ) Map.empty xs
